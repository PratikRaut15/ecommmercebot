...
module.exports.getOrderManagementQuery = (orderId, req, connection) => {
  if (connection !== "" && orderId !== "") {
    ...
    let sql =
      "SELECT order_id,status,product_name,product_image from orders where order_id = '" +
      orderId +
      "' ";
    return sql;
  }
      ...
      db.connection.query(sql, async function (err, res, fields) {
        ...
        if (res.length === 0) {
          ...
          if (
            res.length > 0 &&
            res[0].status &&
            (res[0].status === "delivered" ||
              res[0].status === "out for delivery" ||
...
updateCancelOrder = (orderId, reason) => {
  return new Promise((resolve, reject) => {
    let sql =
      "Insert into cancellation(order_id,reason) values('" +
      orderId +
      "','" +
      reason +
      "')";
...
module.exports.getTrackingStatus = (orderId, reason) => {
  return new Promise((resolve, reject) => {
    let arrData = {};
    let sql =
      "select status,product_name,product_image from orders where order_id = '" +
      orderId +
      "'";
...
module.exports.getContextObject = (req) => {
  if (req.body.queryResult.outputContexts) {
    ...
    for (var i in req.body.queryResult.outputContexts) {
      ...
      if (
        req.body.queryResult.outputContexts[i].name.includes("session_variable")
      ) {
...
getDeliveryDetails = (orderId) => {
  ...
  return new Promise((resolve, reject) => {
    if (orderId !== "") {
      let msg = "";
      let sql =
        "select delivery.agent_name,delivery.agent_contact,delivery.product_name,orders.product_image from delivery inner join orders on orders.order_id = delivery.order_id where delivery.order_id =  '" +
        orderId +
              ...
              [
                {
                  type: "description",
                  title: "Order:" + orderId + "",
                  text: ["No such order has been placed."],
                },
              ],
            ],
          },
...
module.exports.getResponseForTrackingStatus = (data, orderId) => {
  ...
  if (data.response === false) {
              ...
              [
                {
                  type: "description",
                  title: "Order: " + orderId + " - " + data.product_name + "",
                  text: ["Incorrect order ID", "No such order is placed"],
                },
              ],
            ],
          },
     ...
     *  Date-05/03/2021
     */
    if (data.status == "packed") var path = "packed.png";

    if (data.status == "shipped") var path = "shipped.png";

    if (data.status == "delivered") var path = "Delivered.png";

    if (data.status == "cancelled") var path = "cancelled.png";

    var image_url =
      "https://raw.githubusercontent.com/dpmanek/images/main/" + path + "";
    result = {
      fulfillmentMessages: [
        {
          payload: {
  ...
  /** changes made by Nikhil ; changed richcontent types and captured image link to fetch images; Date-05/03/2021   ****/
    ...
    if (data.status == "out for delivery") {
      var image_url =
        "https://raw.githubusercontent.com/dpmanek/images/main/ood.png";
    }
            ...
            richContent: [
              [
                ...
                },
                {
                  ...
                  text: ["Your order will be delivered to you by 9PM"],
                },
              ],
            ],
          },
...
module.exports.getResponseForTrackingComplaintStatus = (
  response,
  incidentno
) => {
